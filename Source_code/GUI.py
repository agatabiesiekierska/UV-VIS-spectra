...
class Program:
    ...
    def _open_options_window(self):
        ...
        if result:
            if result['x_min'].isalnum() and  200 <= float(result['x_min']) <= 700: self.x_min = float(result['x_min'])
            if result['x_max'].isalnum() and 200 <= float(result['x_max']) <= 700: self.x_max = float(result['x_max'])
            if result['y_min'].isalnum() and float(result['y_min']) > 0: self.y_min = float(result['y_min'])
            if result['y_max'].isalnum() and float(result['y_max']) > 0: self.y_max = float(result['y_max'])
